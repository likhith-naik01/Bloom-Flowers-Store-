...
export function ShopProvider({ children }) {
  ...
  const fetchShopData = async (full = true) => {
    try {
      ...
      // ALWAYS fetch /api/orders so orders list is 100% up to date everywhere
      ...
      if (ordRes && ordRes.ok) {
        ...
        if (!initialFetchDone.current) {
          ...
          const unnotifiedNewOrders = fetchedOrders.filter(
            (o) => (o.status === 'placed' || o.status === 'new') && !notifiedOrderIdsRef.current.has(o.id)
          );
  ...
  const refreshData = () => fetchShopData(true);
