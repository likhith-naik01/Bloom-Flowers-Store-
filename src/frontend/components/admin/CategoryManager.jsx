...
export default function CategoryManager({ categories = [], onAddCategory, onEditCategory, onDeleteCategory }) {
  ...
  return (
    <div className="bg-creamCard p-4 rounded-2xl border border-divineGold/35 my-4 space-y-3 shadow-sm">
      {savedMsg && (
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-bold text-darkBrown flex items-center gap-1.5">
          <Layers className="w-4 h-4 text-marigold" /> Categories & Occasions ({categories.length})
        </h2>
        <button
          onClick={() => {
            if (showAdd) {
          ...
          className="px-3 py-1.5 rounded-xl bg-gradient-to-r from-marigold to-templeRed text-creamBg font-bold text-xs flex items-center gap-1 transition-all shadow-md shadow-marigold/20"
        >
      ...
      {showAdd && (
        <form onSubmit={handleSubmit} className="bg-creamSurface p-4 rounded-2xl border border-divineGold/30 space-y-3 shadow-md">
          ...
          <div>
            <label className="text-[11px] font-bold text-darkBrown mb-1 block">English Name * (e.g. Bouquets Section)</label>
            <input
              type="text"
              required
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              <label className="text-[11px] font-medium text-warmMuted mb-1 block">Hindi (Optional)</label>
              <input
                type="text"
                placeholder="e.g. Shadi Mala"
                value={nameHi}
                onChange={(e) => setNameHi(e.target.value)}
                className="w-full p-2 rounded-xl bg-creamCard text-xs text-darkBrown border border-divineGold/30 focus:outline-none"
              />
            </div>
            <div>
              <label className="text-[11px] font-medium text-warmMuted mb-1 block">Kannada (Optional)</label>
              <input
                type="text"
          ...
          <div className="p-3 rounded-xl bg-creamCard border border-divineGold/30 space-y-2">
            <label className="text-xs font-bold text-marigoldDark flex items-center gap-1.5">
              <Upload className="w-4 h-4 text-marigold" /> Category Photo *
            </label>
            ...
            {imageUrl && (
              <div className="relative w-full h-24 rounded-xl overflow-hidden border border-divineGold/30 mt-2">
          ...
          <div className="flex items-center gap-2 pt-1">
            <button
              type="submit"
              className="flex-1 py-3 rounded-xl bg-gradient-to-r from-marigold to-templeRed hover:from-marigoldDark hover:to-templeRedDark text-creamBg font-bold text-xs shadow-md transition-all border border-divineGold/40"
            >
            ...
            {editingCategory && (
              <button
                ...
                className="px-4 py-3 rounded-xl bg-creamCard hover:bg-creamSurface text-darkBrown font-bold text-xs border border-divineGold/30"
              >
                Cancel
              </button>
            )}
      ...
      <div className="space-y-2">
        {categories.map((c) => {
          ...
          return (
            <div key={c.id} className="flex items-center justify-between p-2.5 rounded-xl bg-creamSurface border border-divineGold/25 hover:border-marigold/50 transition-all">
              <div className="flex items-center gap-3">
                <div className="relative w-10 h-10 rounded-lg overflow-hidden flex-shrink-0 border border-divineGold/30 bg-creamCard">
                  {catImg ? (
                    <img src={catImg} alt={c.nameEn || c.name} className="w-full h-full object-cover" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center text-xs text-warmMuted">🌸</div>
                  )}
                </div>
                <div>
                  <h4 className="text-xs font-bold text-darkBrown">{c.nameEn || c.name}</h4>
                  {(c.nameHi || c.nameKn) && (
                    <span className="text-[10px] text-marigoldDark font-medium block">
              ...
              <div className="flex items-center gap-1">
                <button
                  ...
                  className="p-1.5 text-warmMuted hover:text-darkBrown transition-colors"
                  title="Edit Category"
                >
                ...
                <button
                  ...
                  className="p-1.5 text-warmMuted hover:text-templeRed transition-colors"
