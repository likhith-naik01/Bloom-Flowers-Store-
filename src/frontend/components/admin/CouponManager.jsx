  ...
  return (
    <div className="bg-creamCard p-4 rounded-2xl border border-divineGold/35 my-4 space-y-4 shadow-sm">
      ...
      {loading ? (
        ...
        <div className="text-center py-8 bg-creamSurface rounded-xl text-xs text-warmMuted">
          No coupons created yet. Click "+ Create New Coupon" above.
        </div>
      ) : (
          ...
          {coupons.map((c) => (
            <div
              ...
              className={`p-3 rounded-2xl border flex items-center justify-between gap-3 shadow-xs ${
                c.is_active ? 'bg-creamSurface border-divineGold/30' : 'bg-creamCard/50 border-divineGold/15 opacity-70'
              }`}
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1">
                  <span className="px-2.5 py-0.5 rounded-full bg-marigold/20 text-marigoldDark font-extrabold text-xs uppercase tracking-wider border border-marigold/30">
                    {c.code}
                  </span>
                  <span className="text-xs font-bold text-templeRed">
                ...
                <button
                  ...
                  className={`px-2.5 py-1 rounded-full text-[10px] font-bold border transition-colors flex items-center gap-1 ${
                    c.is_active
                      ...
                      : 'bg-warmMuted/15 text-warmMuted border-warmMuted/30'
                  }`}
                >
